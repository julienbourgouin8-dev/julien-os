#!/usr/bin/env node
/*
 * verify.js — visual-verification harness (mechanical, no model).
 *
 ...
 * implement the dev contract described in engine-recipes.md: ?jump=<scrollY> lands
 ...
 *
 * Does not replace a real-device mobile pass — see mobile-reliability.md for bugs that only
 * reproduce on real iOS Safari hardware.
 */
...
async function jank(url) {
  await withBrowser(async b => {
    ...
    console.log(stats.max < 50 ? 'PASS (max < 50ms)' : 'JANK — investigate the bitmap decode/DPR/frame weight');
